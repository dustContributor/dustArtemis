...
public final class EntityGroup {
	...
	private final void rebuildEntityList(final int startId) {
		 ...
		 *
		 * NOTE: It seems explicitly checking for j < ids.length helps the JIT a bit, j
		 * wont ever be bigger than ids.length, but probably the JIT can't infer that
		 * and checks every loop if it has to raise an out of bounds exception.
		 */
		int id;
	...
	/**
	 * Checks if the provided bit set has at least one of the bits set in any bits.
	 *
	 * @param bits to check.
	...
	private final boolean checkAll(final long[] bits, final int offset) {
		...
		/*
		 * If the intersection of the two bit sets is equal to the all bits, it means
		 * the passed bit set has all of all bits set. Otherwise, reject the entity.
		 */
