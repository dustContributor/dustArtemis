...
public final class OpenBitSet implements Cloneable
{
	 ...
	 * x86) vs // a long shift, and a long AND, (the test for zero is prob a
	 * no-op) // testing on a P4 indicates this is slower than (bits[i] & bitmask)
	 * != 0; }
	 */
	 ...
	 * bit = index & 0x0000003f; // mod 64 return (bits[word] << bit) < 0; //
	 * hmmm, this would work if bit order were reversed // we could right shift
	 * and check for parity bit, if it was available to us. }
	 */
	...
	public final void fastClear ( final int index )
	{
		 ...
		 * correct place (using sign extension). Could also use Long.rotateRight()
		 * or rotateLeft() *if* they were converted by the JVM into a native
		 * instruction. bits[word] &= Long.rotateLeft(0xfffffffe,bit);
		 */
	}
	...
	public final void flip ( final int startIndex, final int endIndex )
	{
		...
		/***
		 * Grrr, java shifting wraps around so -1L>>>64 == -1 for that reason, make
		 * sure not to use endmask if the bits to flip will be zero in the last word
	...
	/*
	 * public final static int pop(long v0, long v1, long v2, long v3) { //
